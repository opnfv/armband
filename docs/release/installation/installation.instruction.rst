...
The management bridge(mgmt_br) is required later for functest/yardstick, it is suggested to preconfig it on installer.
The public bridge(public_br) is also nice to have, but not mandatory.

The user running deploy script on the Jumpserver should belong to root and libvirt groups, and have passwordless sudo access.

For aarch64 deployment, the Jumpserver should also have libvirt version >= 3.x, preferably 3.5 or 3.6.
To be able to obtain a newer libvirt package, the "linux.enea.com" repo needs to be added into Jumpserver's repository list.

For example, in Ubuntu OS, create a file "enea.list" under /etc/apt/sources.list.d/ and update the list of available packages
...
#. Baremetal deploy
   ...
   An aarch64 deploy on pod5 from Arm lab
    ...
    In practice:
   - use -S option to point to a tmp dir where the build images are saved. The images can be re-used between deploys
