...
<armband/upstream/config.mk> file.

Note! The same observation as above, before altering Makefile, run:

.. code-block:: bash

    $ make submodules-init patches-impor
...
Only the plugin targets from <armband/upstream/f_isoroot/Makefile> that
...
There is only one preffered method available for building Fuel:
